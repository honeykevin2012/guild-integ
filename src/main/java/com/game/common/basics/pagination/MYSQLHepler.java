...
public class MYSQLHepler {
	...
	public static String getCountString(String querySelect) {
		...
		StringBuilder sBuilder = new StringBuilder(querySelect.length());
		if (select.toLowerCase().indexOf("select distinct") != -1 || querySelect.toLowerCase().indexOf("group by") != -1) {
			sBuilder.append("select count(1) count from (");
			sBuilder.append(querySelect.substring(0, orderIndex));
			sBuilder.append(" ) t");
			return sBuilder.toString();
		} else {
			sBuilder.append("select count(1) count ");
			sBuilder.append(querySelect.substring(formIndex, orderIndex));
			return sBuilder.toString();
		}
	}
	...
	private static int getLastOrderInsertPoint(String querySelect) {
		...
		boolean isAdapter = isBracketCanPartnership(querySelect.substring(orderIndex, querySelect.length()));
		if (orderIndex == -1 || !isAdapter) {
