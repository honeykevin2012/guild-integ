 ...
 * @author kevin
 */
public class MessageUserPracticTemplate extends MessageTemplate{
	private static String NODE_NAME = "/setting/messageUserPractic/messageUserPracticTemplate";
	private static String NODE_TITLE_NAME = "/setting/messageUserPractic";
	private static String SUB_NODE_NAME = "practic";
	private static Map<String, Object> template = new HashMap<String, Object>();
	@Override
	public EntryEnum getEntry() {
		return EntryEnum.PRACTIC;
	}
	...
	public Map<String, Object> getParameters(Object... values) {
		...
		if(!this.getPractics().isEmpty()){
			...
			for(PracticVirtual practic : this.getPractics()){
				practicNames.append(EntryEnum.PRACTIC.getIcon()).append(" ").append(practic.getName()).append(".\r\n");
			}
	...
	public static void main(String[] args) {
		...
		practic1.setName("行车记录仪");
		practic1.setQuantity(1);
		attacts.add(practic1);
		attacts.add(practic);
		core.setAdapter(new MessageUserPracticTemplate()).transfer(attacts, "4", "哈哈哈").send();
	}
}
